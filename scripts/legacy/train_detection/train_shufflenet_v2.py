...
# -*- coding: utf-8 -*-
"""
ShuffleNet V2 分类训练：极轻量模型，覆盖低端手机用户。

用法：
  首次训练：
    python train_shufflenet_v2.py --data /path/to/dataset_cls --epochs 50

  继续训练：
    python train_shufflenet_v2.py --data /path/to/dataset_cls --resume runs/shufflenet_v2/weights/best.pt --epochs 20

  可选 --variant x0_5（更轻）或 x1_0（默认，平衡）。
"""
...
DEFAULT_SAVE_DIR = RUNS_DIR / "shufflenet_v2"

# 支持 x0_5（约 2.3M 参数）与 x1_0（约 2.3M→ 实际 x1_0 更大一点，约 2.3M 输出 1000 类）
VARIANTS = {
...
MODEL_TYPE = "shufflenet_v2"  # 与 API 中 model_type 一致，具体 variant 存在 ckpt 里可选


def get_args():
    p = argparse.ArgumentParser(description="ShuffleNet V2 分类训练（支持从已有权重继续训练）")
    ...
    p.add_argument("--variant", type=str, default="x1_0", choices=list(VARIANTS.keys()), help="x0_5 更轻 / x1_0 默认")
...
def build_model(num_classes: int, variant: str = "x1_0", resume_path: str | None = None):
    """构建 ShuffleNet V2；若提供 resume_path 则加载已有权重。"""
...
def main():
    ...
    print(f"Using device: {device}  model: ShuffleNet V2 {args.variant}")
