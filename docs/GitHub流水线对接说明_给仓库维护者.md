  ...
  "predictions": [
    {
      ...
      "predicted_class": "苹果",
      "raw_class": "apple",
      "predicted_class_zh": "苹果",
      "predicted_class_en": "Apple",
      "confidence": 0.92
    },
    {
      "image": "incoming/category/uploads/20260519_143052_a1b2c3d4.jpg",
...
| `predicted_class` | 是 | **App 界面展示用中文名**（如 `鸭梨`）；勿再填 `yali_pear` 这类内部 key |
| `raw_class` | 强烈建议 | 模型原始类别 id（与 `classes.json` 的 `species_classes` 一致，供调试/对照） |
| `predicted_species_key` | 强烈建议 | 归一化后的物种 key（经 `species_aliases` 映射，如 `carambola`→`starfruit`） |
| `predicted_class_en` | 强烈建议 | 展示用英文名（如 `Yali Pear`） |
| `predicted_class_zh` | 强烈建议 | 与 `predicted_class` 相同的中文名（如 `鸭梨`） |
